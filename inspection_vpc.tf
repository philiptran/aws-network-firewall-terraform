...
resource "aws_route_table" "inspection_vpc_tgw_subnet_route_table" {
  ...
  route {
    ...
    vpc_endpoint_id = element([for ss in module.firewall.firewall_sync_states : ss.attachment[0].endpoint_id if ss.attachment[0].subnet_id == aws_subnet.inspection_vpc_firewall_subnet[count.index].id], 0)
  }
  tags = {
